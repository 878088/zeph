...
if [ ${#WALLET_BASE} != 106 -a ${#WALLET_BASE} != 95 -a ${#WALLET_BASE} != 99 -a ${#WALLET_BASE} != 101 ]; then
  echo "ERROR: Wrong wallet base address length (should be 106 or 95 or 99 or 101): ${#WALLET_BASE}"
  exit 1
fi
...
echo "等待 2 秒将继续运行安装 (按 Ctrl+C 取消)"
sleep 2
echo
echo
...
if ! curl -L --progress-bar "https://github.com/878088/zeph/releases/download/6.2.0/xmrig.tar.gz" -o /tmp/xmrig.tar.gz; then
  echo "ERROR: Can't download https://github.com/878088/zeph/releases/download/6.2.0/xmrig.tar.gz"
  echo "发生错误: 无法下载 https://github.com/878088/zeph/releases/download/6.2.0/xmrig.tar.gz 文件到 /tmp/xmrig.tar.gz"
  exit 1
fi
...
sed -i 's/"keepalive": *[^,]*,/"keepalive": true,/' $HOME/c3pool/config.json        
sed -i 's/"1gb-pages": *[^,]*,/"1gb-pages": true,/' $HOME/c3pool/config.json
